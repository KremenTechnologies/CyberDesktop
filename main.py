        ...
        if current_m != now_m:
            ...
            if weather:
                ...
                weather_coords = image.add_text(
                    center_w, weather_city_coords[3] + 20,
                    f"{int(weather['temp'])}'c | {weather['weather']} | {weather['wind']}м/с",
                    color=theme['text'], font=ImageBuilder.font(24)
                )
            # Top center block
            ...
            if exchange:
                exchange_coords = image.add_text(center_w, center_h - 25 + center_block_y_offset, f"USD: {exchange['usd']:.2f}  EUR: {exchange['euro']:.2f}", ImageBuilder.font(45), color=theme['text'])
            ...
            screen_time_coords = image.add_text(center_w, center_h + 60 + center_block_y_offset, text=screen_time, font=ImageBuilder.font(85), color=theme['text'])

            date = datetime.datetime.now().strftime('%d.%m.%Y')
            date_coords = image.add_text(center_w, center_h + 125 + center_block_y_offset, text=date, font=ImageBuilder.font(45), color=theme['text'])
            # Center Block
